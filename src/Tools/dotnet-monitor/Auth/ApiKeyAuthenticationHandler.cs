...
namespace Microsoft.Diagnostics.Tools.Monitor
{
    ...
    internal sealed class ApiKeyAuthenticationHandler : AuthenticationHandler<ApiKeyAuthenticationHandlerOptions>
    {
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            if (string.IsNullOrEmpty(authenticationOptions.HashType))
            {
                return Task.FromResult(AuthenticateResult.Fail("Missing hash algorithm"));
            }
            if (DisallowedHashAlgorithms.Contains(authenticationOptions.HashType, StringComparer.OrdinalIgnoreCase))
            {
                return Task.FromResult(AuthenticateResult.Fail($"Disallowed hash algorithm {authenticationOptions.HashType}"));
            }

            using HashAlgorithm algorithm = HashAlgorithm.Create(authenticationOptions.HashType);
            if (algorithm == null)
            {
                return Task.FromResult(AuthenticateResult.Fail($"Invalid hash algorithm {authenticationOptions.HashType}"));
            }
